...
class Heroes(UnitsType):
    ...
    def Warrior_unit(self, name):
        return Warrior(name, 30, 15, 10, 5, SAction = CriticalHit())

    def Archer_unit(self, name):
        return Archer(name, 20, 10, 10, 10, SAction = PiercingShot())
...
def TurnAction(attaker, target):
    ...
    if special_active:
        ...
        time.sleep(1.5)
        print(f'💥 ✨ 👹' if attaker in HeroesList else f'👹 ✨ 💥')
        time.sleep(1.5)
        ...
        print(f'😠 ⚔️ 👹' if attaker in HeroesList else f'👹 ⚔️ 😱')
        time.sleep(1.5)
